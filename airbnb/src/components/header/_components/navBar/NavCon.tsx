...
export default function NavCon({
  selectCategory,
  setSelectCategory,
}: NavConProps) {
  ...
  useEffect(() => {
    const storedSelectCategory = localStorage.getItem('selectCategory');
    if (storedSelectCategory) {
      setSelectCategory(storedSelectCategory);
    }
  }, []);
    ...
    <>
      <div className={style.navCon}>
        ...
        <div className={style.etcHeaderNavCon}>
          <Link to="https://www.airbnb.co.kr/host/homes">
      ...
      {openUserSetting && <UserSettingMenu />}
      {openGlobalSetting && (
