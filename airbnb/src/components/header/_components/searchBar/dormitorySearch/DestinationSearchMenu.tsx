...
const tabMenu = [
  { name: '전세계', key: 'allCountries' },
  { name: '동아시아', key: 'eastAsia' },
  { name: '동남아시아', key: 'eastSouthAsia' },
  { name: '유럽', key: 'europe' },
  { name: '아메리카 대륙', key: 'america' },
];

export default function DestinationSearchMenu() {
  const [selectTabMenu, setSelectTabMenu] = useState('전세계');

  const handleTabMenu = (name: string) => {
    setSelectTabMenu(name);
  };

  const currentCountries: Country[] =
    countries[
      tabMenu.find((menu) => menu.name === selectTabMenu)?.key || 'allCountries'
    ];
    ...
    <div className={style.DestinationSearchMenu}>
      <section className={style.searchToRegion}>
        ...
        <ul className={style.regionCards}>
          {regions.map((region, index) => (
            <li key={index} className={style.regionCard}>
              <div>
      ...
      <section className={style.koreaRegion}>
        ...
        <ul className={style.koreaRegionCards}>
          {koreaRegions.map((region, index) => (
            <li key={index} className={style.koreaRegionCard}>
      ...
      <section className={style.allCountriesRegion}>
        ...
        <ul className={style.tabMenuCon}>
          {tabMenu.map((menu, index) => (
            <li
              key={index}
              onClick={() => handleTabMenu(menu.name)}
              className={`${style.tabMenuItem} ${selectTabMenu === menu.name ? style.on : ''}`}
            >
              {menu.name}
            </li>
          ))}
